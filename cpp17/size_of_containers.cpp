...
int main() {
    ...
    static_assert(sizeof(std::any) == 16); // SSO (data 8 bytes + ptr to manager) or non-SSO (ptr to data + ptr to manager)
    ...
    static_assert(sizeof(std::function<std::int64_t(std::int64_t)>) == 32); // ptr to function, invoker, manager + ?
    static_assert(alignof(std::function<std::int64_t(std::int64_t)>) == 8);
